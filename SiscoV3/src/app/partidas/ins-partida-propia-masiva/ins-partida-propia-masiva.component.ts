...
@Component({
  selector: 'app-ins-partida-propia-masiva',
  templateUrl: './ins-partida-propia-masiva.component.html',
  providers: [SiscoV3Service]
})
export class InsPartidaPropiaMasivaComponent implements OnInit,  OnDestroy {
  claveModulo = 'app-ins-partida-propia-masiva';
  datosevent;
  datos = [];
  idTipoObjeto: number;
  ...
  sp = '[partida].[INS_PARTIDACONTRATO_MASIVA_SP]';
  urlApi = 'partida/PostInsPartidaPropiaMasiva';
  subsNegocio: any;
    ...
    this.subsNegocio = this.getStateNegocio.subscribe((stateNegocio) => {
      this.getStateAutenticacion.subscribe((stateAutenticacion) => {
        ...
        if (this.contratoActual !== null) {
          ...
          this.parametrosSP.rfc = this.contratoActual.rfcEmpresa;
  ...
  /**
   * @description Muestra los campos que se requieren para poder insertar partidas propias
                  ...
                  if (this.headers.length > 0) {
                    ...
                    for (const data in this.headers[0]) {
                      ...
                      if (this.headers[0][data] !== 'File' && this.headers[0][data] !== 'Image'
                        ...
                        this.columnas.push({
                          nombre: data, tipo: type, longitud: 500,
                          obligatorio: obligatorio.obligatorio, descripcion: data.charAt(0).toUpperCase() + data.slice(1)
                        });
