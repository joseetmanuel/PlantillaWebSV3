...
import { IGridGeneralConfiguration, TiposdeDato, TiposdeFormato, TipoBusqueda, IBuscador, IObjeto, AccionNotificacion, GerenciaNotificacion } from 'src/app/interfaces';
...
@Component({
  selector: 'app-ins-solicitud-cotizacion',
  templateUrl: './ins-solicitud-cotizacion.component.html',
  styleUrls: ['./ins-solicitud-cotizacion.component.scss']
})

export class InsSolicitudCotizacionComponent implements OnInit {
  // tslint:disable: triple-equals
  idClase = '';
  spinner = false;
  claveModulo = 'app-ins-solicitud-cotizacion';
  idCliente: number;
  ...
  partidasRestantesGrid: IGridGeneralConfiguration;
  ...
  fnEliminaPartidasDetalle(data: { event?: any; data: any; }) {
    ...
    this.workshopGridDetailData = this.workshopGridDetailData.filter(function (el) { return el != null; });
  ...
  fnEliminaSeleccion(data) {
    ...
    this.workshopGridData = this.workshopGridData.filter(function (el) { return el != null; });
    this.partidasReferential.forEach(element => {
        ...
        + this.numeroContrato + '&idCliente=' + this.idCliente + '&idClase=' + this.idClase).subscribe((res: any) => {
          if (res.err) {
            ...
            res.recordsets[0].forEach(element => {
              ...
              this.partidasReferential.push(element);
            });
            this.partidasDataShadow = res.recordsets[0];
  ...
  fnAmountType($event) {
    ...
    const subtotal = this.fnPlusSelectedData();
      ...
      this.siscoV3Service.postService('proveedor/PostObtieneCostosPartidaPorProveedor', parametros).subscribe(
        (res: any) => {
          ...
          if (res.err) {
              ...
              this.partidasSelData.forEach(pElement => {
                if (pElement.idPartida == element.idPartida && this.amountType === 1) {
                  ...
                  dataProveedor = {
                    ...
                    Costo: element.Costo,
                    Venta: element.Venta,
                    idPadre: this.currentCustomer.rfcProveedor,
            ...
            this.partidasDataShadow = this.partidasDataShadow.filter(function (el) {
              return el != null;
            });
  ...
  fnCalculateTotal() {
    const partidasLength = this.partidasDataShadow.length;
    for (let index = 0; index < partidasLength; index++) {
      if (this.amountType == 1) {
        this.partidasDataShadow[index].Total = (Number(this.partidasDataShadow[index].Costo) * this.partidasDataShadow[index].cant);
      } else if (this.amountType == 2) {
        this.partidasDataShadow[index].Total = (Number(this.partidasDataShadow[index].Venta) * this.partidasDataShadow[index].cant);
      }
    }
    const cotLength = this.workshopGridData.length;
    for (let index = 0; index < cotLength; index++) {
      let operacion = 0;
      const detalleLength = this.workshopGridData[index].detalle.length;
      for (let indexDetalle = 0; indexDetalle < detalleLength; indexDetalle++) {
        if (this.amountType == 1) {
          this.workshopGridData[index].detalle[indexDetalle].Total =
            (Number(this.workshopGridData[index].detalle[indexDetalle].Costo) * this.workshopGridData[index].detalle[indexDetalle].cant);
        } else if (this.amountType == 2) {
          this.workshopGridData[index].detalle[indexDetalle].Total =
            (Number(this.workshopGridData[index].detalle[indexDetalle].Venta) * this.workshopGridData[index].detalle[indexDetalle].cant);
        }
        operacion = operacion + this.workshopGridData[index].detalle[indexDetalle].Total;
      }
      this.workshopGridData[index].Total = operacion;
    }
  }
  ...
  editevent($event) {
    if ($event != null) {
      let operacion = 0;
      if (this.amountType == 1) {
        operacion = Number($event.editdata.key.Costo) * Number($event.editdata.newData.cant);
      } else if (this.amountType == 2) {
        operacion = Number($event.editdata.key.Venta) * Number($event.editdata.newData.cant);
      }
      $event.editdata.key.Total = operacion;
      const cotLength = this.workshopGridData.length;
      let sumaTotal = 0;
      for (let index = 0; index < cotLength; index++) {
        let operacionGrid = 0;
        const detalleLength = this.workshopGridData[index].detalle.length;
        for (let indexDetalle = 0; indexDetalle < detalleLength; indexDetalle++) {
          operacionGrid = operacionGrid + this.workshopGridData[index].detalle[indexDetalle].Total;
        }
        this.workshopGridData[index].Total = operacionGrid;
        sumaTotal = sumaTotal + operacionGrid;
      }
      this.fnPlusTotal(sumaTotal);
    }
  }
  ...
  fnPlusPrices(total: number) {
    if (total <= 0) {
      ...
      this.sumaPrecios.subTotal = total.toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,');
      this.sumaPrecios.IVA = (total * 0.16).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,');
      this.sumaPrecios.Total = (total + (total * 0.16)).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,');
    }
  }

  fnPlusTotal(total: number) {
    if (total <= 0) {
      ...
      this.sumaTotal.subTotal = total.toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,');
      this.sumaTotal.IVA = (total * 0.16).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,');
      this.sumaTotal.Total = (total + (total * 0.16)).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,');
    }
  }
  ...
  fnAgregarNuevaCotizacion() {
    ...
    this.siscoV3Service.postService('solicitud/PostInsCotizacion', parametros).toPromise().then((res: any) => {
      this.spinner = false;
      if (res.err) {
        this.Excepciones(res.err, 4);
      } else if (res.excepcion) {
  ...
  fnCreateEspecialidadesXML(array): string {
    ...
    if (array !== null) {
      array.forEach(element => {
        const especialidad = element.Especialidad !== null ? element.Especialidad : 0;
        cadena +=
