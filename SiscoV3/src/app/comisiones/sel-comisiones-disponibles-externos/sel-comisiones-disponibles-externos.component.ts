...
@Component({
  selector: 'app-sel-comisiones-disponibles-externos',
  templateUrl: './sel-comisiones-disponibles-externos.component.html',
  styleUrls: ['./sel-comisiones-disponibles-externos.component.sass']
})
export class SelComisionesDisponiblesExternosComponent implements OnInit {
  // VARIABLES PARA SEGURIDAD
  claveModulo = 'app-sel-comisiones-disponibles-externos';
  idClase = '';
  modulo: any = {};
  // VARIABLES PARA NGRX
  ...
  ConfigParamsDataGrid() {
    this.columns = [{
      ...
      {
        caption: "% Utilidad ",
        dataField: "porcentajeUtilidad",
        dataType: TiposdeDato.number,
        format: TiposdeFormato.percent
      },
      {
        caption: "Comision",
        dataField: "comision",
  ...
  validacionFactura(){
    ...
    this.datosevent.forEach(function (element) {
      totalSeleccionado += element.comision
    });
  ...
  Cobrar(data) {
    try {
      ...
      data.data.forEach(function (element, index, array) {
        cobrar += "<idComision>" + element.idComision + "</idComision>"
      })
  ...
  datosMessage($event) {
    ...
    let totalComision = 0
    this.datosevent.forEach(function (element) {
      totalComision += element.comision
    })
