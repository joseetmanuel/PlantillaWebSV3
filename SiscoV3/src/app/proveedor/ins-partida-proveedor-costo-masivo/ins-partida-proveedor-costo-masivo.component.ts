...
@Component({
  selector: 'app-ins-partida-proveedor.costo-masivo',
  templateUrl: './ins-partida-proveedor-costo-masivo.component.html',
  providers: [SiscoV3Service]
})
export class InsPartidaProveedorCostoMasivoComponent implements OnInit, OnDestroy {
  claveModulo = 'app-ins-partida-proveedor-costo-masivo';
  datosevent;
  datos = [];
  idTipoObjeto: number;
  ...
  sp = '[proveedor].[INS_PARTIDACOSTO_SP]';
  urlApi = 'proveedor/PostProveedorPartidaCostoMasivo';
      ...
      this.getStateAutenticacion.subscribe((stateAutenticacion) => {
        ...
        if (this.modulo.breadcrumb) {
          this.breadcrumb = Negocio.GetConfiguracionBreadCrumb(
            this.modulo.breadcrumb, this.idClase, [{rfcProveedor: this.rfcProveedor, idTipoObjeto: this.idTipoObjeto}]);
        }
  ...
  GetTipoCobro() {
    ...
    this.siscoV3Service.getService('partida/GetPartidaTipoCobro?idClase=' + this.idClase).subscribe((result: any) => {
      ...
      if (result.err) {
        ...
        // Se agrega la columna Id de la partida.
        this.columnas.push({
          nombre: 'Id',
          tipo: TypeData.NUMBER,
          longitud: 500,
          obligatorio: true,
          descripcion: 'Id',
          disabled: true
