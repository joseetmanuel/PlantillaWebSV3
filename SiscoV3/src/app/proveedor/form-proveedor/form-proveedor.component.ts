...
export class FormProveedorComponent implements OnInit, OnDestroy {
  ...
  public tipo = 2;
  public municipios;
  public tipoVialidades: any;
  public tipoAsentamientos: any;
  ...
  ngOnInit() {
    ...
    this.subsNegocio = this.getStateNegocio.subscribe((stateNegocio) => {
      ...
      this.getStateAuth.subscribe((state) => {
        if (state.seguridad && this.idClase) {
          ...
          this.idDocumento = this.datosProveedor ? this.datosProveedor.logo : 0;
          ...
          this.IUploadFile = {
            ...
            descripcion: '', previsualizacion: true, tipodecarga: 'instantly', idDocumento: this.idDocumento
          };
        }
      });
    });
    ...
    if (this.proveedorEntidad) {
      this.newProveedorForm.controls.razonSocial.setValue('razonSocial');
    }
   ...
   * @param datosProveedor Datos del proveedor para mostrar en el formulario.
   * @author Andres Farias
   */
  AddDatosFormProveedor(datosProveedor: IProveedor) {
    this.idDocumento = datosProveedor.logo;
    ...
    this.newProveedorForm.controls.numeroInterior.setValue(datosProveedor.numeroInterior);
     if (this.idDocumento) {
      this.IUploadFile.idDocumento = this.idDocumento;
    }
  }
   ...
   * @param $event valor del evento del input rfcProveedor.
   * @author Andres Farias
   */
   ...
   * @param $event Valor del evento generado del select
   * @author Andres Farias
   */
